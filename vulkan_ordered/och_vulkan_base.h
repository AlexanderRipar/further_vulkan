...
static void vulkan_context_resize_callback(GLFWwindow* window, int width, int height);
...
struct required_feature_list
{
	...
	uint32_t m_dev_extension_cnt = 0;

	uint32_t m_dev_layer_cnt = 0;
...
{
	static required_feature_list feats;



	VkSurfaceCapabilitiesKHR m_surface_capabilites;

	queue_info render_queue_info;

	queue_info transfer_queue_info;
	...
	err_info create(const char* app_name, uint32_t window_width, uint32_t window_height)
	{
		// Create window
		{
			...
			check(feats.add_instance_extensions(glfwGetRequiredInstanceExtensions(&glfw_extension_cnt), glfw_extension_cnt));
		}
		...
		// Create instance
		{
			...
			check(feats.check_instance_support(supports_extensions));
			...
			instance_ci.enabledLayerCount = feats.inst_layer_cnt();
			instance_ci.ppEnabledLayerNames = feats.inst_layers();
			instance_ci.pApplicationInfo = &app_info;
			instance_ci.enabledExtensionCount = feats.inst_extension_cnt();
			instance_ci.ppEnabledExtensionNames = feats.inst_extensions();
		...
		// Select physical device and save queue family info
		{
			...
			for (uint32_t i = 0; i != avl_dev_cnt; ++i)
			{
				...
				bool supports_extensions;
				check(feats.check_device_support(dev, supports_extensions));
				...
				transfer_queue_info = { transfer_queue_index, families[transfer_queue_index] };
				render_queue_info = { render_queue_index, families[render_queue_index] };
		...
		// Create logical device
		{
			...
			queue_cis[0].queueFamilyIndex = render_queue_info.index;
			...
			queue_cis[1].queueFamilyIndex = transfer_queue_info.index;
			...
			device_ci.enabledLayerCount = feats.dev_layer_cnt();
			device_ci.ppEnabledLayerNames = feats.dev_layers();
			device_ci.enabledExtensionCount = feats.dev_extension_cnt();
			device_ci.ppEnabledExtensionNames = feats.dev_extensions();
			device_ci.pEnabledFeatures = &enabled_features;

			check(vkCreateDevice(m_physical_device, &device_ci, nullptr, &m_device));

			vkGetDeviceQueue(m_device, render_queue_info.index, 0, &m_render_queue);

			vkGetDeviceQueue(m_device, render_queue_info.index, 0, &m_transfer_queue);
		}
...
struct swapchain_context
{
	...
	err_info create(const vulkan_context& context)
	{
		// Get swapchain settings supported by context
		{
			...
			uint32_t present_mode_cnt;
			check(vkGetPhysicalDeviceSurfacePresentModesKHR(context.m_physical_device, context.m_surface, &format_cnt, nullptr));
			och::heap_buffer<VkPresentModeKHR> present_modes(present_mode_cnt);
			check(vkGetPhysicalDeviceSurfacePresentModesKHR(context.m_physical_device, context.m_surface, &format_cnt, present_modes.data()));
		...
		{
			// Choose image count
			uint32_t img_cnt = surface_capabilites.minImageCount + 1;
			...
			check(vkCreateSwapchainKHR(context.m_device, &swapchain_ci, nullptr, &m_swapchain));
		}
		...
		// Create swapchain image views
		{
			for (uint32_t i = 0; i != m_image_cnt; ++i)
			{
				...
				imview_ci.viewType;
				imview_ci.format = m_format.format;
