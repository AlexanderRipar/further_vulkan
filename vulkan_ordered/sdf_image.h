...
public:
	...
	struct colour_mapper
	{
		...
		static texel_b8g8r8 monochrome(float dst) noexcept
		{
			constexpr float hi = 0.015625F * 2.0F, lo = -hi;

			const float clamped = dst < lo ? lo : dst > hi ? hi : dst;

			const float scaled = (clamped - lo) * (255.0F / (hi - lo));

			const uint8_t c = static_cast<uint8_t>(scaled);
